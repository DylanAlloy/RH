...
class Robinhood:
    ...
    endpoints = {
        ...
        "ach_transfers": "https://api.robinhood.com/ach/transfers/", #TODO see what we can do about money transfers
        ...
        "notifications": "https://api.robinhood.com/notifications/", #TODO add notifcations
        "orders": "https://api.robinhood.com/orders/",
        "password_reset": "https://api.robinhood.com/password_reset/request/", #TODO maybe?
        ...
        "watchlist_defualt": "https://api.robinhood.com/watchlists/Default/",
        "news": "https://api.robinhood.com/midlands/news/", #TODO maybe populate with either the bloomberg or with this news, or both
    ...
    #Written by Brandon Jackson @DevBrandon Github, @DevBrandonJ Gmail
    def watchlists(self):
        """fetch defualt watchlist """
        res = self.session.get(self.endpoints['watchlist_defualt'])
        res.raise_for_status()  #will throw without auth
        data = res.json() # data is in instrments e.g. {'watchlist': 'https://api.robinhood.com/watchlists/Default/',
        #'instrument': 'https://api.robinhood.com/instruments/8907cd52-1ec6-4ea6-8d34-42b584bbfb9a/',
        #'created_at': '2016-12-12T20:13:49.303302Z', 'url': 'https://api.robinhood.com/watchlists/Default/8907cd52-1ec6-4ea6-8d34-42b584bbfb9a/'}
        #loader = json.loads(data.decode('utf-8')

        return data
    #Written by Brandon Jackson @DevBrandon Github, @DevBrandonJ Gmail
    def watchlist_instruments(self):
        data = self.watchlists()#will throw without auth
        instruments = []
        for i in range(len(data['results'])):
            req = requests.get(data['results'][i]['instrument'])
            req.raise_for_status()
            instrument = req.json()
    ...
    def watchlist_simple(self):
        ...
        for i in range(len(data['results'])):
            ...
            req.raise_for_status()
            instrument = req.json()
            watchlist.append(self.quote_data(instrument['symbol']))

        return watchlist

    '''
    Saving orginal stable version
    #Returns price, name, and tix test
    def watchlist_simple(self):
        data = self.watchlist_instruments()
        watchlist = []
        for da in data:
           watchlist.append(self.quote_data(da['symbol']))
    ...
    def positions_results(self):
        """Returns the user's positions results data."""
        data = self.session.get(self.endpoints['positions']).json()
        return data['results']
    ...
    def instrument_owned(self):
        ...
        for position in positions['results']:
            quantity = float(position['quantity'])
            if quantity > 0:
                securities.append( self.quote_data(self.session.get(position['instrument']).json()['symbol']))
        return securities
    ...
    def place_order(
        ...
        if not bid_price:
            bid_price = float(self.quote_data(instrument[0]['symbol'])['bid_price'])
        url = instrument[0]['url']
        payload = {
            'account': self.get_account()['url'],
            'instrument': url,
            'price': float(bid_price),
            'quantity': quantity,
            'side': 'buy', #transaction.name.lower(),
    ...
    #Written by Brandon Jackson @DevBrandon Github, @DevBrandonJ Gmail
    def place_order_new(self, instrument, quantity=1, side="" ,bid_price = None):
        # cache the account ID that's needed for placing orders
        if self.positions == None:
            self.positions = self.get_endpoint("positions")['results']
        if bid_price == None:
            bid_price = self.quote_data(instrument[0]['symbol'])['bid_price']
        data = "account={0}&instrument={1}&price={2}&quantity={3}&side={4}&symbol={5}&time_in_force=gfd&trigger=immediate&type=market".format(self.get_account()['url'], instrument[0]['url'], float(bid_price), quantity, side,instrument[0]['symbol'])
        res = self.session.post(self.endpoints['orders'], data=data)
        return res

    #Written by Brandon Jackson @DevBrandon Github, @DevBrandonJ Gmail
    def place_order_options(self, instrument, quantity=1, side="" , typebs = None, bid_price = None, trigger= None):
        ...
        if trigger == None:
            trigger = 'immediate'
        #Limit Buy or Sell
        if typebs == 'limit' and trigger != 'stop': #Works for limit Buy ^ Sell
            data = "account={0}&instrument={1}&price={2}&quantity={3}&side={4}&symbol={5}&time_in_force=gfd&trigger={6}&type={7}".format(self.get_account()['url'], instrument[0]['url'], float(bid_price), quantity, side, instrument[0]['symbol'], trigger, typebs)
            res = self.session.post(self.endpoints['orders'], data=data)
            return res
        #Stop Loss Buy or Sell, may need to change order
        if trigger == 'stop' and typebs != 'limit': #Works for Sell
            data = "account={0}&instrument={1}&stop_price={2}&quantity={3}&side={4}&symbol={5}&time_in_force=gfd&trigger={6}&type={7}".format(self.get_account()['url'], instrument[0]['url'], float(bid_price), quantity, side, instrument[0]['symbol'], trigger, typebs)
            print(data)
            res = self.session.post(self.endpoints['orders'], data=data)
            return res

        #Stop Limit Buy or Sell
        if trigger == 'stop' and typebs == 'limit': #Testing
            data = "account={0}&instrument={1}&stop_price={2}&quantity={3}&side={4}&symbol={5}&time_in_force=gfd&trigger={6}&type={7}".format(self.get_account()['url'], instrument[0]['url'], float(bid_price), quantity, side, instrument[0]['symbol'], trigger, typebs)
            print(data)
            res = self.session.post(self.endpoints['orders'], data=data)
            return res
    ...
    def cancel_order(self, id):
        data="{0}/cancel/".format(id)
        data = "https://api.robinhood.com/orders/" + data
        res = self.session.post(data)
        return res

    #Written by Brandon Jackson @DevBrandon Github, @DevBrandonJ Gmail
    def return_queued_orders(self):
        res = self.order_history()['results']
        rs = []
        for r in res:
            if r['state'] == 'queued' or r['state'] == 'confirmed':
                req = requests.get(r['instrument'])
                req.raise_for_status()
                instrument = req.json()
                rt = []
                rt.append(instrument['symbol'])
                rt.append(r['id'])
                rt.append(r['quantity'])
                rt.append(r['side'])
                rt.append(r['updated_at'])
                rs.append(rt)
        return rs
